...
from DSTC2.traindev.scripts.model import mixed
...
if __name__ == "__main__":
    ...
    record_file = codecs.open("threshold_record.txt", 'a', encoding='utf8')
    threshold = 0.1
    plt.figure(1)
    # ready to plot
    thres = []
    ...
    for n in roll:
        ...
        input_mtr, output_mtr = mixed.basic_cnn_LSTM_init(one_set.input_mtr, one_set.output_mtr)


        # Shape output
        X_train, X_test, y_train, y_test = train_test_split(input_mtr, output_mtr, test_size=0.2, random_state=True)
        y_train, y_test = mixed.output_shape(y_train, y_test)

        # TODO:设计降维算法，需要将输出的维度降低。
        # get model
        # model = LSTM.get_basic_LSTM(one_set.sentence_length, len(one_set.act_dict))
        model = mixed.get_mixed(input_mtr.shape[1:], len(y_train[0]))

        # Train
        model.fit(X_train, y_train, batch_size=4, nb_epoch=12)
